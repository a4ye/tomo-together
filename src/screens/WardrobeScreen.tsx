...
export default function WardrobeScreen() {
  ...
  const selectMascot = (id: string) => saveAvatar(id, me.equipped);

  const mascots = items.filter((i) => i.type === 'mascot');
  const accessories = items.filter((i) => i.type !== 'mascot');
  // A price of 0 means every account starts with that mascot unlocked.
  const mascotOwned = (item: WardrobeItem) => item.price === 0 || me.owned.includes(item.id);
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 30 }}>
        ...
        <Text style={{ fontFamily: F.display, fontSize: 15, color: C.brown, marginTop: 18, marginBottom: 2 }}>
          Mascots
        </Text>
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', marginTop: 8 }}>
          {mascots.map((item, i) => {
            const owned = mascotOwned(item);
            const selected = me.color === item.id;
            return (
              <DoodleCard key={item.id} seed={i * 3 + 40} style={{ width: '48%', marginBottom: 12, alignItems: 'center' }}>
                <Avatar color={item.id} equipped={[]} size={76} />
                <Text style={{ fontFamily: F.display, fontSize: 14, color: C.darkInk, marginTop: 2 }}>
                  {item.name}
                </Text>
                {owned ? (
                  <Pressable onPress={() => selectMascot(item.id)} disabled={selected} style={{ marginTop: 6 }}>
                    <View
                      style={{
                        backgroundColor: selected ? C.yellow : C.white,
                        borderWidth: 2.5, borderColor: selected ? C.brown : '#CBD8A0',
        ...
        <Text style={{ fontFamily: F.display, fontSize: 15, color: C.brown, marginTop: 10, marginBottom: 2 }}>
          Accessories
        </Text>
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', marginTop: 8 }}>
